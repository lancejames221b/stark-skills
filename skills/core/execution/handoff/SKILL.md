...
/handoff <MAC_HOST> done with <PROJECT_NAME> RE, need to review YARA rules next
/handoff phase 7+8 done. obsidian live. next: migrate memories (#43)
```
...
TOKEN=$(grep '^DISCORD_TOKEN=' <LOCAL_PATH>/dev/<VOICE_RUNTIME>-voice/.env 2>/dev/null | cut -d= -f2 \
  || ssh -o BatchMode=yes <INFERENCE_HOST> "grep '^DISCORD_TOKEN=' ~/dev/<VOICE_RUNTIME>-voice/.env | cut -d= -f2" 2>/dev/null)
...
- If Notion is unavailable, write Obsidian file under `~/Obsidian/Claude/project/handoff-<date>.md` and store `[HANDOFF-MEM]` with `Obsidian-Path` instead of Notion URL.
- Previous Discord-based `/handoff` slash command (`~/.claude/commands/handoff.md`) is preserved as fallback for sending Discord messages directly — it just no longer carries the primary session state.
