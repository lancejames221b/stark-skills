...
| Symptom | What to do |
|---|---|
| `ssh: connect to host <INFERENCE_HOST>` fails | Confirm Tailscale up; try `ssh <TAILSCALE_IP>` or `ssh <PRIVATE_IP>` directly. If both fail, <INFERENCE_HOST> is offline — activate Tier 2: read `./MEMORY.md` as primary source (see "Project-local MEMORY.md" section above). |
