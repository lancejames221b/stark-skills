...
| `notion` | `notion-create-pages`, `notion-update-page`, `notion-search` | DB id `<NOTION_DATABASE_ID>` for the Memory database. |
...
| Symptom | What to do |
|---|---|
| `ssh: connect to host <INFERENCE_HOST>` fails | Confirm Tailscale up; try `ssh <HOST_ALIAS>` or `ssh <PRIVATE_IP>`. If both fail, refuse to claim save success — buffer in temp and retry, or tell the user the save is queued/blocked. |
