...
### Memory & Organization
- **hAIveMind (via mcporter)**: Reference previous research — use `mcporter call haivemind.search_memories query="[topic]" limit=10` (NOT memory_search — it's broken)
...
**You MUST post research results back to the originating channel before completing.**
...
2. **Format findings** for the platform (Discord: <2000 chars per message; Slack: mrkdwn; WhatsApp/Signal: plain text)
3. **Post results:**
   ```
   message action=send channel=[platform] target=[channel-id] message="🔬 **Research Complete**\n\n[executive summary + key findings]"
   ```
4. **Research reports are always large** — write full report to `<LOCAL_PATH>/dev/tmp/results/research-[topic]-[timestamp].md`
5. Post executive summary + key findings to channel, reference file for full report
6. Include source count and confidence levels in the summary
