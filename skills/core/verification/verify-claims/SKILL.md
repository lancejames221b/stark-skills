...
2. Format results for the platform (Discord: <2000 chars; Slack: mrkdwn; WhatsApp/Signal: plain text)
3. Post via: `message action=send channel=[platform] target=[channel-id] message="[result]"`
4. Large results: write to `<LOCAL_PATH>/dev/tmp/results/`, post summary to channel
5. On failure: post error message — never fail silently
